<template>
  <div class="login">
    ...
    <div class="el-login-footer">
      <span>Copyright © 2023/07/08 Yiming 版权所有.</span>
    </div>
  </div>
</template>
...
<script setup>
  ...
  const handleLogin=()=>{
    //进行信息验证，valid为true验证成功，否则失败
    loginRef.value.validate(async (valid)=>{
      if(valid){
        //必须验证通过之后才进行下列逻辑判断（bug修正20230708）
        ...
        try{
          ...
          if(data.code==200){
            //把token，set到store（vuex）里面去
            const token = data.authorization
            const menuList=data.menuList; //菜单树
            const perms=data.perms;
            const currentUser=data.currentUser;//获取当前用户
...
.login-form {
  border-radius: 6px;
  background-color: rgb(208, 163, 72);
  width: 400px;
